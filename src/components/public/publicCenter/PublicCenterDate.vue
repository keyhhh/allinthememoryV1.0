  ...
  >
    <template #header>
      <div class="card-header">
        ...
        <span>
          <el-tag
            v-for="(index, id) in card.tag.length"
            :key="index"
            class="ml-2"
            type="success"
            >{{ card.tag[id] }}</el-tag
...
import { getRecordPublicByDate, likeRecordPublic } from "@/axios/public";
...
export default {
  setup() {
    ...
    //传递过来的日期参数
    const searchDateValue = route.query.searchDateValue;
    let record = {
      userId: store.state.user.userId,
      datePublish: searchDateValue,
    };
    ...
    const isLikeImg = (event, card, cardIndex) => {
      ...
      if (
        ...
        //不是点赞状态，设置点赞
        publicRecord[cardIndex].likedUserId[
    ...
    getRecordPublicByDate(record).then((res) => {
      if (res.data.code) {
