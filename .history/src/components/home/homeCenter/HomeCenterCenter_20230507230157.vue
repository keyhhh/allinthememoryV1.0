  ...
  <el-timeline>
    ...
    >
      <el-card>
        ...
        <div v-for="index in record.image.length " :key="index">
          <img  :src= "require('../../../assets/ALMIMG/' + record.image)"  style="width: 45%; margin-right: 5%" />
        </div>
    ...
    getRecord(store.state.user).then((res) => {
      ...
      if (res.data.code) {
        for (let index = 0; index < res.data.data.length; index++) {
          recordData.push({
            datePublish : res.data.data[index].userId,
            image : res.data.data[index].userId,
            isMemory : res.data.data[index].userId,
            isPublic :res.data.data[index].userId,
            message = res.data.data[index].userId,
            recordId = res.data.data[index].userId,
            tag = res.data.data[index].userId,
            userId = res.data.data[index].userId,
          });
          recordData.push(res.data.data[index].image.split(","));
