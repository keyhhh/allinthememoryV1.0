  ...
  <el-timeline>
    ...
    >
      <el-card>
        ...
        <!-- 删除按钮 -->
        <el-button type="danger" @click="deleteRecord(index,$event)">删除</el-button>
        <!-- 公开开关 -->
        <el-radio-group
          style="float: right"
...
import { getRecord, isOpen } from "@/axios/home"; //获取记录api
export default {
  setup() {
    ...
    function radioChange(index,value) {
      ...
      let record = {//创建新的record
        recordId:recordData[index].recordId,
        isPublic:value
      }
      console.log("record",record);
      isOpen(record).then((res) => {//监视是否设置为公开私密
        if (res.data.code) {
          console.log("切换成功");
        } else {
          console.log("切换失败");
     ...
     * 删除记录
     */
     function deleteRecord(index,value) {
      // console.log("index", index,"value",value);//index是找到哪个记录，value是改变后的值
      // console.log(recordData[index].recordId);//获取改变的记录id
