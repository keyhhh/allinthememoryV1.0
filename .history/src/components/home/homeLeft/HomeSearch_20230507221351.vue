<template>
  <div class="home-search">
    ...
    <el-dialog
      ...
      title="Describe your recordShow"
    >
      <el-form :model="recordShow">
        <!-- 标题输入框 -->
        <!-- <el-form-item label="recordShowTittle" :label-width="formLabelWidth">
          <el-input
            v-model="recordShow.title"
            autocomplete="off"
            placeholder="请输入记忆标题"
          />
        </el-form-item> -->
        ...
        <!-- 是否公开switch 
            class="ml-4"
            label="recordShowOpen"
            v-model="recordShow.isOpen"-->
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <!-- 保存添加记录 -->
          <el-button type="primary" @click="commitrecordShow"> 保存 </el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
import { uprecordShow } from "@/axios/home";
...
export default defineComponent({
  ...
  setup() {
    ...
    const commitrecordShow = () => {
      console.log("进入方法");
      // const proxy = getCurrentInstance();
      //判断数据是否存在空，
      if (recordShow.message == "") {
        //回去接着写
        console.log("不可以不可以提交");
        dialogFormVisible.value = true;
      } else {
        //提交
        console.log("走了else", pictureUpload.value, "recordShow", recordShow);
        pictureUpload.value.submit(); //手动上串图片
        recordShow.userId = localStorage.getItem("userInfo"); //把上传记录的用户id传过去
        let record = {
          datePublish: this.recordShow.datePublish,
          userId: this.recordShow.datePublish,
          message: this.recordShow.datePublish,
          tag: this.recordShow.datePublish,
          image: this.recordShow.datePublish,
          isPublic: this.recordShow.datePublish,
          isMemory: this.recordShow.datePublish,
        };
        uprecordShow(record).then((res) => {
          if (res.data.code) {
            console.log(res);
          } else {
            console.log(res);
    ...
    function handleRemove(UploadFile) {
      ...
      console.log(UploadFile, "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@", dialogImageUrl);
    }
    ...
    const onUploadSuccess = (response) => {
      ...
      recordShow.image.push(response.data);
    };
