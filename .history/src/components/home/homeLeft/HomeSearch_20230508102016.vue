...
export default defineComponent({
  ...
  setup() {
    ...
    const formFileList = [];// 显示的上传文件
    function handleRemove(file) {
      console.log("@arr@", pictureUpload.value.UploadFiles, "#file#", file);
      // let index = arr.indexOf(file)
      // this.imageUrls.splice(index,1)
      // let num = 0
      // pictureUpload.value.UploadFiles.map(item =>{
      //   if(item.uid == file.uid){
      //     pictureUpload.value.UploadFiles.spilc(num,1);
      //   }
      //   num ++
      // })
...
爽朗
    ...
    return {
      ...
      onUploadSuccess,
      pictureUpload,
      formFileList,formFileList: [], // 显示的上传文件
