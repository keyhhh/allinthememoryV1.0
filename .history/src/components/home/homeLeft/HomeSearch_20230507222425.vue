    ...
    >
      <el-form :model="record">
        ...
        <el-form-item label="IsMemory" :label-width="formLabelWidth">
          <el-radio-group v-model.number="record.isMemory" class="ml-4">
        ...
        <el-form-item label="IsOpen" :label-width="formLabelWidth">
          <el-radio-group v-model.number="record.isPublic" size="small">
  ...
  setup() {
    ...
    const commitRecord = () => {
      ...
      if (recordShow.message == "") {
        ...
        //提交
        console.log("走了else", pictureUpload.value, "record", record);
        pictureUpload.value.submit(); //手动上串图片
        record.userId = localStorage.getItem("userInfo"); //把上传记录的用户id传过去
        let record = {
          ...
          isPublic: this.record.isPublic,
          isMemory: this.record.isMemory,
        };
    ...
    const onUploadSuccess = (response) => {
      ...
      record.image.push(response.data);
    };
