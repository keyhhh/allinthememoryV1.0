...
import { getReRecord, getRe } from "@/axios/collectivity";
    ...
    const commitRecord = () => {
      ...
      if (reRecordShow.message == "" || reRecordShow.tag == "") {
        ...
        let recordRelatives = {
          ...
          called: reRecordShow.called,
          image: reRecordShow.image.join(","),
        };
        //上传记录的API
        getReRecord(recordRelatives).then((res) => {
          if (res.data.code) {
            console.log();
          } else {
            console.log();
    ...
    getRe(store.state.user).then((res) => {
      if (res.data.code) {
        ...
        for (let index = 0; index < res.data.data.length; index++) {
          userRe.push({
            relatives_id: res.data.data[index].relatives_id,
        ...
        calls.push({
          value: userRe[0].called,
          label: userRe[0].called,
        })
        
        console.log("userReuserReuserRe", userRe[0].called);
      } else {
        console.log();
      }
    });
