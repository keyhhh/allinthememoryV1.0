<template>
  <div class="capsule-item" v-for="x in 10   " :key="x" @click="toCapsuleDetail($event,value)">
    <span style="font-size: 20px;">{{ capsuleItemTittle }} </span><br />
      <h5>剩余解锁时间:{{  }}</h5>
...
export default {
  ...
  setup() {
    ...
    const value2 = ref(dayjs().add(1, 'month').startOf('month'))    //倒计时

    return { capsuleItemTittle,openCapsuleSrc, lockImgSrc, toCapsuleDetail, value2 };
  },
};
</script>
