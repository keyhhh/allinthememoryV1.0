<template>
  <div
    class="capsule-item"
    v-for="x in cpasuleItem"
    :key="x"
    @click="toCapsuleDetail($event, value)"
  >
    <span style="font-size: 20px">{{ x.tittle }} </span><br /><br />
    ...
    <!-- 胶囊样式图片 -->
    <img v-if="x.lockOrNot" class="capsule-img" src="@/assets/胶囊开蓝.png" />
...
export default {
  ...
  setup() {
    ...
    const cpasuleItem = reactive([ //模拟的胶囊数据
      // { tittle: "致给自己测试", lockOrNot: false },
      // { tittle: "致给自己测试", lockOrNot: true },
    ]);
    ...
    getCapsule(store.state.user).then((res) => {
      ...
      if (res.data.code) {
        cpasuleItem.push({
        // userId:
        capsuleId: res.data.data.capsuleId,
        title: res.data.data.capsuleId,
        dateCreate:res.data.data.capsuleId,
        dateSend:res.data.data.capsuleId,
        typeSend:res.data.data.capsuleId,
        isLocked:res.data.data.capsuleId,
        message:res.data.data.capsuleId,
        });
      } else {
        console.log("不对啊！！");
      }
