...
export default {
  setup() {
    ...
    const shortcuts = [
    {
        ...
        value: () => {
          const date = new Date();
          date.setTime(date.getTime() + 3600 * 1000 * 23);
          return date;
        },
      },
      {
    ...
    const commitCapsule = () => {
      ...
      //1.判断用户是否绑定手机号
      if (store.state.loginUser.phone === "") {
        ElMessage.error("您还未绑定手机号码，无法创建");
      } else {
        //2.判断数据是否存在空，
        if (
          capsuleForm.title == "" ||
          ...
          console.log("capsulecapsule",capsule);
          //3发送上传胶囊信息请求
