<template>
  <!-- 胶囊tittle -->
  <h3>胶囊标题:{{ capsuleShow.capsuleTittle }}</h3>
  <!-- 胶囊建立时间 -->
  ...
  <!-- 胶囊发送渠道 -->
  <h3>胶囊发送渠道:{{ capsuleShow.capsuleTittle }}</h3>
</template>
  ...
  setup() {
    ...
    getCapsuleById(capsule).then((res) =>{
      if (res.data.code) {
        ...
        capsuleShow.typeSend= res.data.data.typeSend,
        capsuleShow.dateSend= res.data.data.dateSend,
        capsuleShow.isLocked= res.data.data.isLocked === 1 ? "发送邮件" : "发送短信",
        capsuleShow.message= res.data.data.message
        console.log("详细界面的饭hi之2",capsuleShow);
      }else{
        console.log();
      }
    })
