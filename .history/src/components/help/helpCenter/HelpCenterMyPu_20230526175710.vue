      ...
      >
        <div v-if="help.image != ''">
          <img
              v-for="index in 2"
              :key="index"
              :src="require('../../../assets/ALMIMG/' + help.image[index - 1])"
        ...
        <div style="padding: 14px">
          ...
          <div class="bottom">
            <time class="help-little-msg" style="width: 70%;line-height: 20px;text-indent:2em;"
          ...
          <el-popconfirm
            ...
            icon-color="#626AEF"
            title="确定要删除吗?"
            @confirm.stop="deleteHelp(help.helpId, $event)"
            @cancel.stop="cancelEvent"
          >
            <template #reference>
              <el-button style="margin-top: 5%" type="danger"  @click.stop=""
                >删除</el-button
              >
            </template>
          </el-popconfirm>
...
import { getHelpMyPu, deHelp } from "@/axios/help";
...
export default {
  setup() {
    ...
    function deleteHelp(helpId) {
      ...
      deHelp(help).then((res) => {
        if (res.data.code) {
          helpData.splice(0, helpData.length);
          for (let index = 0; index < res.data.data.length; index++) {
            helpData.push({
              helpId: res.data.data[index].helpId,
              image: res.data.data[index].image.split(","),
    ...
    return { helpData, toShowhelp, deleteHelp, cancelEvent };
  },
};
</script>
