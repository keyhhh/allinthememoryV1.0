  ...
  <el-row type="flex" justify="center">
    ...
    >
      <el-card
        ...
        @click.stop="toShowhelp($event, help.id)"
      >
        <img
...
export default {
  setup() {
    ...
    function toShowhelp(event, helpMsgId) {
      router.push({
        name: "help",
        query: {
      ...
      if (res.data.code) {
        for (let index = 0; index < res.data.data.length; index++) {
          helpData.push({
            ...
            title:res.data.data[index].image,
            message:  res.data.data[index].image,
            tag: res.data.data[index].image,
