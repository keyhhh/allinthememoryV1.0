<template>
  <div class="help-detail-div">
    ...
    <div class="help-detail-footer" style="float:buttom ;">
      <h2> </h2>
      <div class="discuss" v-if="helpDiscussShow[0]">
        <h3 v-for="(index, id) in helpDiscussShow.length" :key="index">{{helpDiscussShow[id].message}}</h3>
      </div>

      <el-card class="discuss-card">
    <div v-for="(index, id) in helpDiscussShow.length" :key="index" class="text item">{{helpDiscussShow[id].message}}</div>
  </el-card>
    </div>
  </div>
</template>
