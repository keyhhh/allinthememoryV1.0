...
export default {
  setup() {
    ...
    const isLikeImg = (event, card, cardIndex) => {
      ...
      //判断是否为点赞状态
      if(publicRecord[cardIndex].likedUserId.indexOf(localStorage.getItem("userInfo")) != -1){
        console.log("我是点赞状态，请不要点击我");
      }else{
        console.log("点吧点吧点吧",publicRecord[cardIndex].liskeImgSrc);
      }
      let record = {
        //传递后端，记录id和喜欢的用户的id
