<template>
  <el-card class="box-card" v-for="card,cardIndex in publicRecord" :key="card.recordId">
    <template #header>
...
export default {
  setup() {
    ...
    const isLikeImg = (event, card, cardIndex) => {
      ...
      let record = { //传递后端，记录id和喜欢的用户的id
        recordId : publicRecord[cardIndex].recordId,
        likedUserId : publicRecord[cardIndex].likedUserId[publicRecord[cardIndex].likedUserId == null ? 0 : publicRecord[cardIndex].likedUserId.length ]="5"
      }
