...
export default {
  setup() {
    ...
    let liskeImgSrc = ref(liskeImgSrcGNo);
    ...
    const isLikeImg = (event, card) => {
      ...
      likeRecordPublic().then((res) => {//点赞的方法
        if (res.data.code) {
          console.log();
        } else {
          console.log();
      ...
      if (res.data.code) {
        for (let index = 0; index < res.data.data.length; index++) {
          ...
          publicRecord.push({
            ...
            likedUserId: res.data.data[index].likedUserId == null ? null: res.data.data[index].likedUserId.split(","),
            liskeImgSrc: res.data.data[index].likedUserId.split(",").indexOf(localStorage.getItem("userInfo"))> 0? ref(liskeImgSrcRYes) : ref(liskeImgSrcGNo),
          });
        }
      } else {
    ...
    return { publicRecord, isLikeImg, liskeImgSrc };
  },
};
</script>
