    ...
    <el-col :span="19" id="userinfo-center">
      <el-card class="box-card">
        ...
        >
          ...
          <el-descriptions-item>
            <el-button type="danger" @click="toLoginOut">退出登录</el-button>
          </el-descriptions-item>
        </el-descriptions>
      </el-card>
    </el-col>
    ...
    <el-form :model="record">
      ...
      <el-form-item label="紧急联系人电话" :label-width="formLabelWidth">
        <el-tooltip
          class="box-item"
          effect="dark"
          content="用于一年未登录，发送账号密码短信给此号码，不过会在前段时间发给您提醒"
          placement="bottom"
        >
          <el-input
...
import { updateUser, loginOut } from "@/axios/home";
    ...
    const commit = () => {
      if (user[0].userName === "") {
        ...
        if (
          ...
          //下边这一条判断，是让密码要么输入就满足规范，要么就不输入
          (!regPassword.test(user[0].password) && user[0].password != null && user[0].password === NonNullable)
        ) {
          //不满足规范的情况
          ElMessage.error(
            "注意输入电话密码规范，密码长度6-12个字符，包含数字字母"
          );
        } else {
          //满足规范的情况，上传数据修改
