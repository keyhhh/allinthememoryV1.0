...
import { register } from "@/axios/home"; //引入api
...
export default {
  ...
  data: function () {
    return {
      ...
      store: useStore(),
      birthday: new Date(),
      avator:'avatarDefault.png', //从本地设置的默认的头像
      gender: true, //true是男、false是女
    };
  },
  methods: {
    doRegister: function () {
      ...
      let user = {
        ...
        password,
        birthday,
        avator : "avatarDefault.png"
      };
      let regAccount = /^[0-9]{6,12}$/; //正则表达式，6-12位数字
      let regPassword = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,12}$/; //密码长度6-12个字符，包含数字字母
      //第一步：判断是否输入空值
      if (
        account === "" ||
        password === "" ||
      ...
      //第二步：检测用户名密码是否合法
        ...
        ElMessage.error("用户名为6-12位数字");
      } else if (!regPassword.test(password)) {
        ElMessage.error("密码长度8-16个字符，至少各一个大、小写字母，一个数字");
      }
      //第三步：判断两次密码一致不
      ...
      //第三步：检测账号
        ...
        register(user).then((res) => {
          //账号不存在，创建完毕直接跳转，跳转的具体代码和登录界面的一样，设置localStorage、vuex，跳转
          if (res.data.code) {
            const userMessage = res.data.data;
            localStorage.setItem("userInfo", userMessage.userId);
            this.$store.commit("SetLoginUser", userMessage);
            this.$store.commit("SetUser",res.data.data.userId)
            this.router.push({
              path: "/home",
            });
          } else {
